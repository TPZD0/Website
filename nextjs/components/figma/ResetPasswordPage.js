...
export function ResetPasswordPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let resp;
      try {
        resp = await fetch(`${apiBase}/api/auth/reset`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ token, new_password: password }),
        });
      } catch (_err) {
        resp = await fetch(`/api/auth/reset`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ token, new_password: password }),
        });
      ...
      // Redirect to login with reset flag
        ...
        <div className="w-full max-w-md">
          <Card className="flex w-full flex-col gap-6 rounded-[32px] border-0 bg-white/90 p-10 shadow-[0px_24px_80px_rgba(15,23,42,0.12)] backdrop-blur">
            <div className="text-center">
              <h2 className="text-3xl font-semibold text-[#101431]" style={{ fontSize: '2.25rem' }}>Reset Password</h2>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4 text-left">
              <div className="space-y-2">
                <Label htmlFor="reset-password" className="text-base font-medium text-[#101431]" style={{ fontSize: '1.125rem' }}>
                  New Password
                </Label>
                <Input
                  id="reset-password"
              ...
              <div className="space-y-2">
                <Label htmlFor="reset-confirm" className="text-base font-medium text-[#101431]" style={{ fontSize: '1.125rem' }}>
                  Confirm Password
                </Label>
                <Input
                  id="reset-confirm"
